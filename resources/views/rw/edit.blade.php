...
@section('content')
    <div class="container-fluid">
        <h1 class="h3 mb-4 text-gray-800">Edit Ketua RW</h1>

        <div class="card shadow mb-4">
            <div class="card-body">
                <form method="POST" action="{{ route('ketuarw.update', $ketuaRw->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="nama_ketua_rw">Nama Ketua RW</label>
                        <input type="text" class="form-control" id="nama_ketua_rw" name="nama_ketua_rw"
                            value="{{ $ketuaRw->nama_ketua_rw }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="telepon_ketua_rw" name="telepon_ketua_rw"
                            pattern="62\d{11}" placeholder="62xxxxxxxxxxx" value="{{ $ketuaRw->telepon_ketua_rw }}"
                            required>
                    </div>
                        ...
                        <select class="form-control" id="id_perumahan" name="id_perumahan" required>
                            @foreach ($perumahans as $perumahan)
                                <option value="{{ $perumahan->id_perumahan }}"
                                    {{ $ketuaRw->id_perumahan == $perumahan->id_perumahan ? 'selected' : '' }}>
